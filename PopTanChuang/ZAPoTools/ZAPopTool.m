...
@implementation ZAPopTool

+ (ZAPopTool *)sharedInstance {
    static ZAPopTool *sharedClient = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedClient = [[ZAPopTool alloc]init];
    });
    return sharedClient;
}
...
- (void)drawRect:(CGRect)rect{
    ...
    if ([ZAPopTool sharedInstance].shadeBackgroundType == ShadeBackgroundTypeSolid) {
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    if ([ZAPopTool sharedInstance].tapOutsideToDismiss) {
          [[ZAPopTool sharedInstance] hideAnimated:YES withCompletionBlock:nil];
    }
}
...
- (instancetype)init {
    ...
    self = [super initWithFrame:(CGRect){15, -15, 32, 32}];
    if (self) {
        
        [self setBackgroundImage:[UIImage imageNamed:@"down"] forState:UIControlStateNormal];
